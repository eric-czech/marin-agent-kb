...
marin GCS. Runs as a CPU-only iris entrypoint that spins Zephyr workers.
Reference: `experiments/protein/prep_quality_splits.py`.
...
PROCESSED_SUBPATH = f"<area>/processed/<dataset>/<name>-{DATA_VERSION}"
out = f"{marin_prefix()}/{PROCESSED_SUBPATH}/{suffix}/"
```
...
`PROCESSED_SUBPATH`, and `NUM_OUTPUT_SHARDS` (default 8).
...
ZephyrContext(name=..., resources=ResourceConfig(cpu=1, ram="8g"),
              max_workers=4).execute(pipeline)
```
...
- Cap `max_workers` to the target CPU pool (e.g. us-east5 ≈ 6 small VMs);
  oversubscribing evicts the Zephyr coordinator mid-run.
- Submit **one iris job per config/doctype** (`-e RUNS …`) rather than threading
  buckets in-process.
